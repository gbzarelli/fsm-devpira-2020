...
from pokecatcher.actions import hidden_to_visible_action
...
if __name__ == '__main__':
    fsm_factory = StateMachineFactory(
        ...
        action_method=hidden_to_visible_action)
    ).add_transaction(StateMachineTransaction(
        state=States.VISIBLE,
        event=Events.OBSERVE,
        target=States.VISIBLE)
    ).add_transaction(StateMachineTransaction(
        state=States.VISIBLE,
        event=Events.ATTACK,
        target=States.INJURED)
    ).add_transaction(StateMachineTransaction(
